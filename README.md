...
关于 Java 泛型和 ASM 技术，我打算深究 fastjson 的反序列化器对于泛型处理部分的代码后再撰写。
...
事实上，大多数语言都是支持以上这种机制的：所有函数式语言都支持，C++ 有基于括号操作符重载的仿函数，JavaScript 中的函数事实上是闭包，就连传统的 C 可以勉强将函数指针当作“对象”，golang 更是摆脱了这方面的束缚……而之所以我会说在这里我将再次提到“泛型”，就是因为基于泛型，Java 有了和仿函数很相似的“函数对象”，这就是我在前面提到的泛型的更强大的作用。我想我会用函数对象写一个看起来很漂亮的策略模式案例，然后重新对泛型报以谨慎的态度。
